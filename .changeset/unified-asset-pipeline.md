...
Unified asset pipeline — one `AssetsStore` for themes AND stamps, with the
...
byte-identical `page`/`text` brick helper. The default theme/stamp DATA moved out
of `@facet/react` (`DEFAULT_THEME`) and the retired `@facet/kit` (`KIT_STAMPS`)
into a new node-free package, so a second renderer can consume the same defaults.

- `@facet/assets` (new): node-free default-asset DATA (deps = `@facet/core` only)
  — the token value maps, `COLOR`, `DEFAULT_THEME`, and `DEFAULT_STAMPS`
  (hero/card/cta-button as validated `FacetStamp` trees). The single,
...
- `@facet/runtime`: `loadAssets` seeds the `@facet/assets` defaults through the
  ...
  the per-field overlay), stamps union with a custom name shadowing a same-named
  default. An empty/absent store still resolves the defaults, and the "never
  throws" contract now covers the primary store I/O + malformed shapes too.
- `@facet/quickstart`: resolves assets through `loadAssets` on EVERY boot (a
  `MemoryAssets` fallback when no `--assets`), so the default theme + stamp
