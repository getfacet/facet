 ...
 * the one place pixels and hex codes live; the agent never sees them. Swap these
 * maps to reskin every Facet page without touching a single agent. `@facet/react`
 * imports these as its default floor, so this is now the SINGLE source of truth
 * for the default token values (no per-renderer copy).
 */
