...
- Build product-quality defaults with section, card, button, tabs, nav, table, chart, metric, keyValue, badge, progress, alert, list, divider, form, filterBar, emptyState, and loading when those components are allowed. "stat" is legacy compatibility; prefer metric for new KPI and summary values.
- Use input for raw inputs, button for actions, tabs/nav for local navigation, table/chart for display-only data, form/filterBar for input surfaces, and metric/keyValue/badge/progress/alert/list/divider/emptyState/loading for compact product state before assembling equivalent box/text clusters. There is no standalone search node: for a search box with a submit, compose an input (input:"search") with a button whose onPress "collect" points at the container holding the input, so the typed query reaches you on submit.
