 ...
 * it to the environment, which jsdom turns into a window `error` event. So
 * `expect(() => fireEvent.change(…)).not.toThrow()` passes even when the handler
 * threw. Listening for the report is what makes the escape an assertion.
 */
