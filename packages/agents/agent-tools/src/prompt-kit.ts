...
export const FACET_PROMPT_KIT = [
  ...
  `publish_data accepts at most ${BOUNDS.publishDataPayloadChars} authored JSON characters and returns a descriptor. For a new binding, that descriptor is not visible markup or completion: publish once, then immediately mutate markup to bind the exact path; never republish unchanged data. If current markup already binds that path, republishing updates UI without a markup rewrite.`,
