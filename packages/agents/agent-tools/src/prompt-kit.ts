...
- Box layout stays closed tokens and fixed choices, never raw CSS: basis holds a split pane or horizontal-shelf item at a chosen width, columns:"auto" builds a responsive grid whose item floor comes from itemWidth (pair the two), maxHeight bounds a box to its own scrolling viewport, and collapse is row-only ("none" or "stack") — "stack" reflows a row to a column at a narrow viewport. You stay the primary narrow-adaptation authority: when an event reports a narrow viewport, re-author layout with patches; collapse:"stack" is a pre-authored fallback at that same narrow threshold, so a row already carrying it reflows without an agent turn. Below that threshold a resolved collapse:"stack" wins over direction:"row", so to keep a row unstacked at narrow width patch collapse:"none", not direction.
- Use get_brick_spec before authoring unfamiliar media icon fields, table alignment, table width, emptyLabel, chart lineStyle, chart axis, or local style paths; call get_style_choices for unfamiliar style values such as textWrap, lineClamp, dividers, stickyHeader, axisColor, gridColor, labelColor, basis, itemWidth, maxHeight, or collapse.
