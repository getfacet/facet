...
import {
  createLruMap,
  isTreeShaped as isTreeShapedBase,
  type FacetSession,
  type LruMap,
  type VisitorContext,
} from "@facet/core";
 ...
 * session. (Stricter than the core base, which only checks root/nodes; a
 * persisted blob has to clear the server path too.) */
function isTreeShaped(value: unknown): boolean {
  if (!isTreeShapedBase(value)) return false;
  // The base guard already confirmed a plain object with a string `root` and a
  // non-null, non-array `nodes` map; drill into what the offline path reads.
  ...
  const rootNode = nodes[value.root];
...
export class FileStageStore implements StageStore {
  private readonly cache: LruMap<FacetSession> = createLruMap(MAX_CACHED_SESSIONS);
  ...
  async get(agentId: string, visitorId: string): Promise<FacetSession | undefined> {
    ...
    const cached = this.cache.get(key); // a hit touches recency (LruMap.get)
