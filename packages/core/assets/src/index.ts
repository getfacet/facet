 ...
 *
 * Four symbols, and they are the whole root surface. Every other module in
 * this package is private: `theme-default.ts` holds the token values, the four
 * `specs-*.ts` modules hold the component specs, `catalog.ts` composes them,
 * and `ui-patterns.ts` holds component-aware composition guidance. None is a
 * package entry point, and the barrel names them explicitly rather than
 * re-exporting a module wholesale, so what this package publishes is decided
 * here and cannot widen by accident (D-12: no `export *`, anywhere).
 *
...
export { DEFAULT_UI_PATTERN_SET } from "./ui-patterns.js";
