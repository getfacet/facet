 ...
 *
 * Three symbols, and they are the whole root surface. Every other root-side
 * module is private: `theme-default.ts` holds the token values, the five
 * `specs-*.ts` modules hold the component specs, and `catalog.ts` composes
 * them. None of the seven is a package entry point, and the barrel names them
