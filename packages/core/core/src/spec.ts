...
Agent-facing authoring order: composition -> component -> primitive fallback. Primitive bricks are the universal base and fallback: box, text, media, field. Components are locked Facet/platform-managed node shapes for common UI; the catalog tells you which components, variants, themes, and compositions are available for this session. Intrinsic components are locked: button, section, card, tabs, nav, table, chart, metric, keyValue, badge, progress, alert, list, divider, form, search, filterBar, emptyState, loading. "stat" remains legacy compatibility; prefer metric for new KPI and summary values.

Node types (the ONLY allowed types):
- box:   { "id", "type":"box", "children":[ids], "variant"?:name, "style"?:BoxStyle, "hidden"?:bool, "backdrop"?:<mediaNodeId>, "onPress"?:Action, "onHold"?:Action } — primitive container and fallback layout. Flow-only layout (no absolute positioning). "onHold" is a secondary long-press gesture (same Action union as onPress) — never make hold the only path to critical content. "backdrop" references a media node id to paint as a bounded BACKGROUND layer behind this box's flow children (renderer-owned two-layer, no overlap/z-index; the id MUST be a media node); flow children stay in normal flow.
...
- table: { "id", "type":"table", "columns":[{"key":name,"label":string,"align"?:"start"|"center"|"end","sortable"?:boolean}], "rows":[objects], "from"?:datasetName, "caption"?, "variant"?:name } — display-only capped tabular data. Mark a column "sortable":true to let the visitor click its header to reorder the rows locally (ascending -> descending -> unsorted) with no agent turn; the renderer owns the closed comparator, you author no sort logic, and the current sort rides the visitor's next event so you can see how they sorted.
...
Containers are box, section, card, and form. Tables, charts, search, and filterBar are display/control-only: no client-side fetch, query expression, endpoint, resolver, or browser business logic you author. The only local reordering is a table's built-in "sortable" columns — a closed renderer-owned comparator you opt into with a flag, never a sort engine or comparator you write. Backend work stays with the agent through actions/tools and patching new data into the stage (including the "data" warehouse).
...
Landing/hero layouts: the landing-grade tokens compose full-bleed marketing sections from the same closed vocabulary. A full-height hero is a box with "backdrop" (a media node id) + minHeight:"screen" holding a large-size headline (add a "highlight" run) and a call-to-action button; use sticky for a sticky nav bar, scheme:"dark" for dark bands, gradient for gradient backgrounds, and backdropScrim to keep copy legible over a backdrop image. Every one is a CLOSED token NAME the theme resolves to the concrete look — never raw CSS, pixels, or hex.
