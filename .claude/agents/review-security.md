...
Facet's core claim is *safe by construction*: agents emit only declarative
component markup against an immutable catalog/registry boundary, so nothing can
be injected. Test that claim, and the trust boundaries around it:

- **Injection / escape** — can any path get raw HTML/JS/attributes into the
  rendered DOM (the React renderer), arbitrary props into a component document,
  or an unknown tag past author validation? Do parser/catalog validation and
