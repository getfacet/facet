export const meta = {
  name: 'refactor-audit',
  description: 'Facet structural audit — fan out one audit-structure reviewer per dimension over the whole tree, adversarially verify each finding with a severity-tiered vote panel, then rank a cleanup plan by impact/effort.',
...
// refactor-audit: parallel Audit per dimension → tiered adversarial Verify panel per finding → ranked Plan.
// Audit is fanned out by dimension (5 lenses); Verify is fanned out by VOTE (independent skeptics per
// finding, tiered by severity). Reuses audit-structure + review-verifier. Invoked via
// Workflow({name:'refactor-audit', args?:{votes?,thorough?}}). Returns a plan; maintainer approves + executes.
...
return {
  ...
  stats: { dimensions: DIMENSIONS.length, candidates: audited.flat().length, confirmed: confirmed.length, votesHigh: VOTES_HIGH },
}
