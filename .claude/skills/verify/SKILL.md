...
- `test` — `vitest run` (unit tests in grouped
  `packages/{core,renderers,agents,adapters,tools}/*` workspaces and `apps/**`,
  including the `@facet/react` jsdom render tests — `.test.tsx` files with a
...
- **package layout** — the Node regression suite plus
  `scripts/check-package-layout.mjs` pin the exact 15 package name-to-path map,
