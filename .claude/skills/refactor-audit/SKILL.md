...
dimension → per-finding adversarial verify → ranked plan). Unlike `/code-review`
(correctness of a change), this looks at the *shape* of the code and what should
be consolidated. The audit logic lives in the `audit-structure` subagent and
...
dimension, effort, score, evidence, fix, verifierReason}], executionOrder,
doNotTouch }`. Render:
